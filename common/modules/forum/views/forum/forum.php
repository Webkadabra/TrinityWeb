...
<?php if (User::can(Rbac::PERM_CREATE_THREAD) && (User::can(Rbac::PERM_CREATE_THREAD_IN_CLOSED_CATEGORY,['category' => $model->category]) || $model->category->create_thread)): ?>
            <li><a href="<?= Url::to(['forum/new-thread', 'cid' => $model->category->id, 'fid' => $model->id]) ?>" class="btn btn-primary btn-sm"><span class="glyphicon glyphicon-plus"></span> <?= Yii::t('view', 'Create new thread') ?></a></li>
<?php endif; ?>
