...
namespace common\modules\i18n\helpers;

use Yii;
use common\modules\i18n\services\Scanner;
use common\modules\i18n\bundles\TranslationPluginAsset;
...
class Language
{
     ...
     *      ],
     * ];
     * $result = \common\modules\i18n\helpers\Language::a($array, $params);
     * ~~~
     *
     * The result:
     *
    ...
    {
        return Yii::$app->session->has(\common\modules\i18n\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \common\modules\i18n\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \common\modules\i18n\models\LanguageSource();
    ...
    {
        $languageSources = \common\modules\i18n\models\LanguageSource::find()->select('category')->distinct()->all();
