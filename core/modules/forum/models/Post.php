...
use core\modules\forum\models\db\PostActiveRecord;
...
class Post extends PostActiveRecord
{
    ...
    protected function addNewWords($allWords)
    {
        try {
            ...
            foreach ($query->each() as $vocabularyFound) {
                if (($key = array_search($vocabularyFound['word'], $allWords, true)) !== false) {
                    unset($newWords[$key]);
                }
            }
            $formatWords = [];
    ...
    protected function insertWords()
    {
        try {
            ...
            if (!empty($vocabulary)) {
                if (!Podium::getInstance()->db->createCommand()->batchInsert(
                        '{{%vocabulary_junction}}', ['word_id', 'post_id'], $vocabulary
                    )->execute()) {
    ...
    protected function updateWords()
    {
        try {
            ...
            if (!empty($vocabulary)) {
                if (!Podium::getInstance()->db->createCommand()->batchInsert(
                        '{{%vocabulary_junction}}', ['word_id', 'post_id'], array_values($vocabulary)
                    )->execute()) {
                    throw new Exception('Words connections saving error!');
                }
            }
            $queryJunction = (new Query())->from('{{%vocabulary_junction}}')->where(['post_id' => $this->id]);
            foreach ($queryJunction->each() as $junk) {
                if (!array_key_exists($junk['word_id'], $vocabulary)) {
                    if (!Podium::getInstance()->db->createCommand()->delete(
                            '{{%vocabulary_junction}}', ['id' => $junk['id']]
                        )->execute()) {
