...
use core\modules\forum\models\db\PollActiveRecord;
...
class Poll extends PollActiveRecord
{
        ...
        if (!empty($votes)) {
            ...
            try {
                if (!Podium::getInstance()->db->createCommand()->batchInsert(
                        '{{%poll_vote}}', ['poll_id', 'answer_id', 'caster_id', 'created_at'], $votes
                    )->execute()) {
    ...
    public function podiumDelete()
    {
        $transaction = static::getDb()->beginTransaction();
        try {
            if (!Podium::getInstance()->db->createCommand()->delete('{{%poll_vote}}', ['poll_id' => $this->id])->execute()) {
