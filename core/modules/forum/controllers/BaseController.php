...
use core\modules\forum\traits\FlashTrait;
use Exception;
use Yii;
use yii\base\Action;
 ...
 *
 * @property \core\modules\forum\Podium $module
 */
class BaseController extends YiiController
{
    ...
    public function upgradeCheck($warnings)
    {
        ...
        $result = Helper::compareVersions(
            ...
            explode('.', $this->module->podiumConfig->get('forum.version'))
        );
        if ($result === '>') {
        ...
        try {
            ...
            if (!$this->module->user->isGuest) {
                ...
                if ($user && !empty($user->userProfile->timezone)) {
                    $this->module->formatter->timeZone = $user->userProfile->timezone;
                }
            }
        } catch (Exception $exc) {
