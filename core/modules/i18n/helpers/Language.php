...
class Language
{
     ...
     *      ],
     * ];
     * $result = \core\modules\i18n\helpers\Language::a($array, $params);
     * ~~~
     *
     * The result:
     *
    ...
    {
        return Yii::$app->session->has(\core\modules\i18n\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \core\modules\i18n\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \core\modules\i18n\models\LanguageSource();
    ...
    {
        $languageSources = \core\modules\i18n\models\LanguageSource::find()->select('category')->distinct()->all();
